...
class PublicUserApiTests(TestCase):
    ...
    def get_token_pair(self) -> tuple:
        """Helper to get valid access and refresh tokens."""
        user_details = {
            'name': 'Test User',
    ...
    def test_user_with_role_success(self):
        """Test creating a user with a specific role is successful."""
        payload = {
            'email': 'user@example.com',
            'password': 'testpass',
            'name': 'Test User',
        ...
        self.assertTrue(user.check_password(payload['password']))
        self.assertNotIn('password', res.data)
        self.assertEqual(user.role, Roles.EDITOR)
    ...
    def test_refresh_token_invalid(self):
        ...
        invalid_refresh = 'thisisnotavalidtoken'

        res = self.client.post(TOKEN_REFRESH_URL, {'refresh': invalid_refresh})
        self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)
        self.assertNotIn('access', res.data)
...
class PrivateUserApiTests(TestCase):
    ...
    def test_update_user_profile_no_change_role(self):
        """Test updating the user profile for authenticated user."""
        payload = {'role': 'editor'}
        res = self.client.patch(ME_URL, payload)
